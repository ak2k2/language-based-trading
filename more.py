...
def add_indicator(dataframe):
    ...
    ind1 = o_api.request("Line of Pandas code to add a column named " + indicator + " to 'dataframe'. Dataframe already exists and has four columns, 'Open', 'High', 'Low', and 'Close.")
    print(ind1)
    exec(ind1)
    return dataframe
...
#add the added_LS 'Long_Signal' column from the added_LS dataframe to a seperate plot

plotly_plot = final_plot.join(added_LS['Long_Signal'])
...
# for row in final_plot.iterrows():
#     if final_plot["Long_Signal"][row] == 1:




# MAKE A PLOTLY PLOT OF FINAL DATAFRAME
